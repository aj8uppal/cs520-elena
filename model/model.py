...
class EleNa :
    ...
    def load_map(self):
        ...
        with open("graph.pkl", "rb") as f:
            G = pkl.load(f)
        with open("graph_projected.pkl", "rb") as f:
            G_proj = pkl.load(f)
    ...
    def shortest_path_custom(self):
        ...
        self.best_path = EleNa.get_path(parent, self.destination_node, path)
        best_path_lat_long = []
        if self.best_path:
...
'''
#A sample code to initialize all objects and run it with the different algorithms we have. 
